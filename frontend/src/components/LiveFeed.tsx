...
export function ObservationDensity({ results, airportFilter, compact }: { results: AnalysisResult[]; airportFilter: string; compact?: boolean }) {
  ...
  const data = uM(() => {
    ...
    const buckets = Array.from({ length: 8 }, (_, i) => ({
      ...
      start: now - (8 - i) * 15 * 60_000,
    }));
    const scope = airportFilter === "all" ? results : results.filter(r => r.airport_code === airportFilter);
    scope.forEach(r => {
      const ts = new Date(r.timestamp.endsWith("Z") ? r.timestamp : r.timestamp + "Z").getTime();
      const age = (now - ts) / (15 * 60_000);
  ...
  const maxCount = Math.max(1, ...data.map(b => b.critical + b.high + b.medium + b.low));
  const W = compact ? 80 : 120, H = compact ? 20 : 28, bw = W / 8 - 1;
  const sevColors = { critical: "#ff4444", high: "#ff8800", medium: "#e3b341", low: "#44aaff" };
          ...
          return (
            <g key={i}>
              {sevOrder.map(sev => {
                ...
                const bh = Math.max(3, (count / maxCount) * H);
                yOff -= bh;
                return (
                  <rect key={sev} x={i * (bw + 1)} y={yOff} width={bw} height={bh}
                    fill={sevColors[sev]} opacity={0.8} rx={1} />
                );
              })}
              {/* Empty bar outline */}
              {(b.critical + b.high + b.medium + b.low === 0) && (
                <rect x={i * (bw + 1)} y={H - 2} width={bw} height={2}
                  fill="#21262d" rx={1} />
              )}
            </g>
          );
